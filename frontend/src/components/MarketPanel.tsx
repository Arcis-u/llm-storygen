...
import FloatingText, { FloatingMessage } from "@/components/FloatingText";
...
export default function MarketPanel({ items, playerEconomy, onBuyAction }: Props) {
  ...
  const handleBuy = (e: React.MouseEvent, item: ShopItem) => {
    ...
    const newMsg: FloatingMessage = {
      ...
      text: `-${item.price} ${item.currency_type}`,
      color: "#ef4444", // Red for spending
      x: e.clientX,
      y: e.clientY,
    };
    setFloatingMsgs(prev => [...prev, newMsg]);

    // Call parent handler
    onBuyAction(item.item_id);
  };

  return (
    <>
      <FloatingText messages={floatingMsgs} onComplete={(id) => setFloatingMsgs(prev => prev.filter(m => m.id !== id))} />
